...
class TestListProducts:
    ...
    async def test_pagination(self, async_client: AsyncClient):
        ...
        response = await async_client.get(
            PRODUCTS_URL, params={"page": 1, "page_size": 2}
        )
        data = response.json()["data"]
        assert len(data["items"]) == 2
        assert data["total"] == 5
        ...
        response = await async_client.get(
            PRODUCTS_URL, params={"page": 2, "page_size": 2}
        )
        data = response.json()["data"]
        assert len(data["items"]) == 2
        assert data["page"] == 2
        ...
        response = await async_client.get(
            PRODUCTS_URL, params={"page": 3, "page_size": 2}
        )
        data = response.json()["data"]
    ...
    async def test_filter_by_category(self, async_client: AsyncClient):
        ...
        response = await async_client.get(
            PRODUCTS_URL, params={"category_id": c1["id"]}
        )
        data = response.json()["data"]
        assert data["total"] == 2
